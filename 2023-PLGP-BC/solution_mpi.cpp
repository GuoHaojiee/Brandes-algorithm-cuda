...
void run(graph_t *G, double *result)
{
    ...
    /* GPU buffer over-provisioning：理论上界 BATCH_SIZE*loc_m 在 R-MAT
     * (幂律图、多 BFS 同 level 撞上 hub) 下可能恰好达到边界，atomicAdd
     * 累加到 max_out 时没有 bounds check 会 OOB → GPU 内存破坏 (rmat-13)
     * 或临近 cuda allocation 数据污染 (rmat-14 Wrong Answer)。加 4 倍
     * 安全边际，CPU 和 GPU 端同步加大。
     * 影响：GPU 内存增加 4x（rmat-14/4p 时约 350MB → 1.4GB，仍远小于
     * P100 16GB），CPU 端 go_* buffer 同步加大但不影响算法。 */
    static const int GPU_OVER = 4;
    ...
    /* 上传本地 CSR 到 GPU（把 framework 的整型数组转 int） */
    {
        ...
        /* 第三个参数（batch_size）传 GPU_OVER * BATCH_SIZE，让 brandes_gpu.cu
         * 内部按 4x 分配 s_d_out_* buffer。kernel 调用时仍传实际 batch_sz。 */
        bc_gpu_init(loc_n, loc_m, GPU_OVER * BATCH_SIZE,
    ...
    /* GPU 输出缓冲：与 GPU 端 4x 安全边际同步 */
    int max_out = GPU_OVER * BATCH_SIZE * max(loc_m, 1);
